const SpeakerSection = () => {
  return (
    <div>
      <section className="py-16 md:py-24 " id="speakers">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-3xl font-bold text-center mb-12">Speakers</h2>
          <h3 className="text-2xl font-bold text-primary text-center mb-8">
            Plenary Speakers
          </h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 md:gap-12 max-w-2xl mx-auto mb-16">
            <div className="text-center">
              <img
                alt="Photo of Plenary Speaker 1"
                className="w-40 h-40 rounded-full mx-auto mb-4 object-cover shadow-lg"
                src="Proffesors\Kaushik.jpg"
              />
              <h4 className="text-lg font-bold">Prof. Kaushik Chatterjee </h4>

              <p>IISC Banglore</p>
            </div>
            <div className="text-center">
              <img
                alt="Photo of Plenary Speaker 2"
                className="w-40 h-40 rounded-full mx-auto mb-4 object-cover shadow-lg"
                src="Proffesors\JKT_2.jpg"
              />
              <h4 className="text-lg font-bold">Prof. Jitendra Thakur</h4>

              <p>ICGEB New Delhi</p>
            </div>
          </div>
          <h3 className="text-2xl font-bold text-primary text-center mb-8">
            Invited Speakers
          </h3>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
            <div className="text-center">
              <img
                alt="Photo of Invited Speaker"
                className="w-32 h-32 rounded-full mx-auto mb-3 object-cover shadow-md"
                src="Proffesors\Rodriguez.jpeg"
              />
              <h4 className="font-semibold">Prof Rodriguez Manzano</h4>
              <p>Imperial College, London</p>
            </div>
            <div className="text-center">
              <img
                alt="Photo of Invited Speaker"
                className="w-32 h-32 rounded-full mx-auto mb-3 object-cover shadow-md"
                src="Proffesors\Prashant.jpeg"
              />
              <h4 className="font-semibold"> Prof. Prashant Agarwal</h4>
              <p>Northumbria University,UK </p>
            </div>
            <div className="text-center">
              <img
                alt="Photo of Invited Speaker"
                className="w-32 h-32 rounded-full mx-auto mb-3 object-cover shadow-md"
                src="Proffesors\simonCork.webp"
              />
              <h4 className="font-semibold">Prof Simon C Cork</h4>
              <p>Anglia Ruskin University,UK</p>
            </div>
            <div className="text-center">
              <img
                alt="Photo of Invited Speaker"
                className="w-32 h-32 rounded-full mx-auto mb-3 object-cover shadow-md"
                src="Proffesors\Sriparna.jpeg"
              />
              <h4 className="font-semibold">Prof. Sriparna Chatterjee</h4>
              <p> IMMT, Bhubaneshwar</p>
            </div>
            <div className="text-center">
              <img
                alt="Photo of Invited Speaker"
                className="w-32 h-32 rounded-full mx-auto mb-3 object-cover shadow-md"
                src="Proffesors\abhilash.webp"
              />
              <h4 className="font-semibold"> Prof. Abhilash Patel</h4>
              <p>IIT Kanpur</p>
            </div>
            <div className="text-center">
              <img
                alt="Photo of Invited Speaker"
                className="w-32 h-32 rounded-full mx-auto mb-3 object-cover shadow-md"
                src="Proffesors\rizvi.jpg"
              />
              <h4 className="font-semibold"> Prof. Suhail Rizvi</h4>

              <p>IIT Hyderabad</p>
            </div>
            <div className="text-center">
              <img
                alt="Photo of Invited Speaker"
                className="w-32 h-32 rounded-full mx-auto mb-3 object-cover shadow-md"
                src="Proffesors\chainmaya.jpeg"
              />
              <h4 className="font-semibold"> Prof. Chinmay Mahapatra</h4>

              <p>NIT Raipur</p>
            </div>
            <div className="text-center">
              <img
                alt="Photo of Invited Speaker"
                className="w-32 h-32 rounded-full mx-auto mb-3 object-cover shadow-md"
                src="Proffesors\AvinashEranki.jpg"
              />
              <h4 className="font-semibold"> Prof. Avinash Eranki</h4>
              <p>IIT Hyderabad</p>
            </div>
          </div>
          <div className="flex flex-wrap sm:flex justify-center gap-20 mt-10">
            <div className="text-center">
              <img
                alt="Photo of Invited Speaker"
                className="w-32 h-32 rounded-full mx-auto mb-3 object-cover shadow-md"
                src="Proffesors\Indresh-Yadav.png"
              />
              <h4 className="font-semibold"> Prof. Indresh Yadav</h4>
              <p>IIT Bhubaneshwar</p>
            </div>
            <div className="text-center">
              <img
                alt="Photo of Invited Speaker"
                className="w-32 h-32 rounded-full mx-auto mb-3 object-cover shadow-md"
                src="Proffesors\Kaushik.jpeg"
              />
              <h4 className="font-semibold"> Prof. Kaushik Choudhury</h4>
              <p>AIC-IPR</p>
            </div>
            <div className="text-center">
              <img
                alt="Photo of Invited Speaker"
                className="w-32 h-32 rounded-full mx-auto mb-3 object-cover shadow-md"
                src="Proffesors\gawri.jpeg"
              />
              <h4 className="font-semibold"> Prof. Gowri Balachandra</h4>
              <p>IIT BHU</p>
            </div>
          </div>
        </div>
      </section>
    </div>
  );
};

export default SpeakerSection;
